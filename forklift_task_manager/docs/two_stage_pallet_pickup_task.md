...
现在新增“到 N 号库位取托盘”的业务流程。车辆到达目标库位前后，不能直接下降到 N 层插叉，而是先将叉臂升到 `N+3` 的高度，方便雷达识别托盘位置；雷达输出托盘相对车体的前后偏移和横向偏移后，再下降到 N 层取货高度，并根据偏移量完成对准和插叉。
...
库位配置文件：`forklift_task_manager/config/pallet_slots.yaml`
